...
- [NetTraffic-Sentinel](#nettraffic-sentinel)
  ...
  - [快速开始](#快速开始)
    ...
    - [第三步：修改配置](#第三步修改配置)
- [方法二：查看有流量的网卡（推荐，找到 RX/TX 字节数非零的那个）](#方法二查看有流量的网卡推荐找到-rxtx-字节数非零的那个)
- [方法三：查看有默认路由的网卡（最直接）](#方法三查看有默认路由的网卡最直接)
- [示例输出：default via 192.168.1.1 dev eth0](#示例输出default-via-19216811-dev-eth0)
- [^^^^](#)
- [这就是你要填的网卡名](#这就是你要填的网卡名)
    - [第四步：构建并启动](#第四步构建并启动)
    - [第四步：构建并启动](#第四步构建并启动-1)
    - [第五步：访问仪表盘](#第五步访问仪表盘)
  - [环境变量配置详解](#环境变量配置详解)
  - [环境变量配置详解](#环境变量配置详解-1)
  - [时区配置](#时区配置)
  ...
  - [API 接口文档](#api-接口文档)
    ...
    - [`GET /api/query` ⭐ 核心查询接口](#get-apiquery--核心查询接口-1)
